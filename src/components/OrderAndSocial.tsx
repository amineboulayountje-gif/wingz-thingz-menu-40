...
const OrderAndSocial = () => {
  const { order, isComplete, orderSummaryText, resetOrder } = useOrder();
  const whatsappNumber = "1234567890";

  const hasItems = !!order.menu;
  const itemCount = (order.menu ? 1 : 0) + order.sides.length + (order.drink ? 1 : 0);
    ...
    <>
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="space-y-0.5 text-xs sm:text-sm text-foreground">
                  {order.menu && (
                    <p>🍗 {order.menu.name} · <span className="text-primary font-semibold">{order.menu.price}</span></p>
                  )}
                ...
                {!isComplete && (
                  <p className="text-[10px] text-muted-foreground mt-1">
                    {!order.menu ? "Kies een gerecht" : order.sides.length < 2 ? `Kies nog ${2 - order.sides.length} bijgerecht${order.sides.length === 1 ? "" : "en"}` : "Kies een drankje"}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <a
                  href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(orderSummaryText)}`}
                  target="_blank"
                  ...
                  className={`inline-flex items-center gap-2 font-semibold text-sm px-5 py-2.5 rounded-lg transition-all duration-300 shadow-lg ${
                    isComplete
                      ? "bg-[#25D366] hover:bg-[#20bd5a] text-primary-foreground hover:scale-105"
      ...
      <section className={`py-10 sm:py-16 px-4 ${hasItems ? "pb-32" : ""}`}>
        <div className="max-w-md mx-auto text-center space-y-6 sm:space-y-8">
          <div>
            <p className="text-muted-foreground text-xs sm:text-sm mb-3 sm:mb-4">Volg ons</p>
            <div className="flex items-center justify-center gap-4 sm:gap-5">
              <a href="https://snapchat.com" target="_blank" rel="noopener noreferrer"
                ...
                aria-label="Snapchat"><SnapchatIcon /></a>
